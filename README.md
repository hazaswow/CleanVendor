...
- **Auto-skip to the merchant window**: talking to an NPC whose only gossip option is to vend (no quests, no other gossip choices) opens the merchant window directly instead of showing that one-option gossip page. Hold **Shift** while talking to the NPC to see the gossip page as normal instead. Toggle it off entirely from the addon's own options panel.
- Full item tooltip on hover; **Ctrl+left-click** to preview an item on your character (dressing room), Shift+left-click to link it in chat
